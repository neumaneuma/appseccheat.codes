...
bp = Blueprint(
    "vulnerabilities_sqli-second-order", __name__, url_prefix=f"{VULNERABILITIES_PREFIX}/sqli-second-order"
)
...
def change_password():
    ...
    try:
        query = text(
            f"UPDATE sqli-second-order_users SET password = :new_password WHERE username = '{username_from_database}' AND password = :old_password"
        )
    ...
    return (f"Success (2/2) - passphrase: {local_flags.PASSPHRASE['sqli-second-order']}", 200) if change_password_successful else ("Failure", 400)
