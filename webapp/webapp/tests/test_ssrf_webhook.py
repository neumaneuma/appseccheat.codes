...
from ..vulnerabilities import VULNERABILITIES_PREFIX, ssrf_webhook
...
class Ssrf1Tests(unittest.TestCase):
    def setUp(self):
        ...
        self.vulnerable_submit_webhook_path = f"{VULNERABILITIES_PREFIX}{submit_webhook_path}"

    def test_empty_custom_url_returns_400(self):
        response = self.test_client.post(self.patched_submit_webhook_path, follow_redirects=True)
        self.assertEqual(response.status_code, 400)
        response = self.test_client.post(self.vulnerable_submit_webhook_path, follow_redirects=True)
        self.assertEqual(response.status_code, 400)

    def test_first_hint_patched_endpoint_returns_400(self):
        data = {"custom_url": "http://127.0.0.1"}
        response = self.test_client.post(self.patched_submit_webhook_path, data=data, follow_redirects=True)
        self.assertEqual(response.status_code, 400)
    ...
    def test_first_hint_vulnerable_endpoint_returns_202(self):
        data = {"custom_url": "http://127.0.0.1"}
        response = self.test_client.post(self.vulnerable_submit_webhook_path, data=data, follow_redirects=True)
