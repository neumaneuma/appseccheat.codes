...
from ..vulnerabilities import VULNERABILITIES_PREFIX, sqli_second_order
from . import SECRET_KEY, cookie_helper
...
class Sqli2Tests(unittest.TestCase):
    def setUp(self):
        ...
        self.vulnerable_get_username_path = f"{VULNERABILITIES_PREFIX}{get_username_path}"
        self.vulnerable_register_path = f"{VULNERABILITIES_PREFIX}{register_path}"
        self.vulnerable_change_password_path = f"{VULNERABILITIES_PREFIX}{change_password_path}"
    ...
    def test_register_empty_username_and_password_returns_401(self):
        response = self.test_client.post(self.patched_register_path, follow_redirects=True)
        self.assertEqual(response.status_code, 401)
        response = self.test_client.post(self.vulnerable_register_path, follow_redirects=True)
        self.assertEqual(response.status_code, 401)

    def test_register_empty_username_returns_401(self):
        data = {"password": "password"}
        response = self.test_client.post(self.patched_register_path, data=data, follow_redirects=True)
        self.assertEqual(response.status_code, 401)
        response = self.test_client.post(self.vulnerable_register_path, data=data, follow_redirects=True)
        self.assertEqual(response.status_code, 401)
    ...
    def test_change_password_no_cookies_400(self):
        response = self.test_client.post(self.patched_change_password_path, follow_redirects=True)
        self.assertEqual(response.status_code, 400)
        response = self.test_client.post(self.vulnerable_change_password_path, follow_redirects=True)
        self.assertEqual(response.status_code, 400)
    ...
    def test_change_password_username_cookie_missing_400(self):
        ...
        self.test_client.set_cookie(self.patched_change_password_path, self.session_cookie_name, session_cookie)
        response = self.test_client.post(self.patched_change_password_path, follow_redirects=True)
        self.assertEqual(response.status_code, 400)
        self.test_client.set_cookie(self.vulnerable_change_password_path, self.session_cookie_name, session_cookie)
        response = self.test_client.post(self.vulnerable_change_password_path, follow_redirects=True)
        self.assertEqual(response.status_code, 400)
    ...
    def test_change_password_no_cookies_missing(self):
        ...
        try:
            self.test_client.post(self.patched_change_password_path, follow_redirects=True)
        except Exception:
        ...
        exception_thrown = False
        try:
            self.test_client.post(self.vulnerable_change_password_path, follow_redirects=True)
        except Exception:
            exception_thrown = True
