...
from ..vulnerabilities import ssrf_lfi
...
class Ssrf1Tests(unittest.TestCase):
    ...
    def test_empty_custom_url_returns_400(self):
        response = self.test_client.post(
            self.patched_submit_api_path, follow_redirects=True
        )
        self.assertEqual(response.status_code, 400)
        response = self.test_client.post(
            self.vulnerable_submit_api_path, follow_redirects=True
        )
        self.assertEqual(response.status_code, 400)
    ...
    def test_first_hint_patched_endpoint_returns_400(self):
        ...
        response = self.test_client.post(
            self.patched_submit_api_path, data=data, follow_redirects=True
        )
        self.assertEqual(response.status_code, 400)
    ...
    def test_first_hint_vulnerable_endpoint_returns_202(self):
        ...
        response = self.test_client.post(
            self.vulnerable_submit_api_path, data=data, follow_redirects=True
        )
